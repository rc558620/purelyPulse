...
const isPartnerMatched = (
  ...
  if (provinceLabel && !(partner.province || '').includes(provinceLabel)) {
    return false;
  }

  if (cityLabel && !(partner.city || '').includes(cityLabel)) {
    return false;
  }

  if (districtLabel && !(partner.district || '').includes(districtLabel)) {
    return false;
  }
...
export const usePromotionDetailDerived = ({
  ...
  }, [data.partners.length, data.regions, hasClientFilter, matchedPartnerProvinceSet, submittedQuery]);

  const regionSummary = useMemo(() => filteredRegions.reduce<RegionSummary>((summary, regionItem) => ({
    totalPartners: summary.totalPartners + regionItem.partnerCount,
    totalOrders: summary.totalOrders + regionItem.totalOrders,
    totalRevenue: summary.totalRevenue + regionItem.totalRevenue,
  }), EMPTY_REGION_SUMMARY), [filteredRegions]);
